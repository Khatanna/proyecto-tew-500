...
@section('content')
  ...
  <div class="col-12">
    <form action="" class="row g-3 my-2">
      <label for="" class="col-md-10">
    ...
    @foreach($docentes as $index => $docente)
      <tr>
        ...
        " data-bs-toggle="modal" data-bs-target="#modal{{$docente->id}}" type="button">
          ...
          <form action="{{route('admin.dashboard.docentes.destroy')}}" method="post" class="d-inline">
            @csrf
            @method('delete')
            <input type="text" name="id" value="{{$docente->id}}" hidden="">
